/***************************************************************************
 * Importante: Ver settings.h para configurar el seteo de la aplicacion!!!
 * ************************************************************************/
#include "settings.h"
...
void connectWifi() 
{
  ...
  //Manual Wifi
  Serial.printf("Connectando a WiFi %s/%s", WIFI_SSID.c_str(), WIFI_PASS.c_str());
  WiFi.disconnect();
  WiFi.mode(WIFI_STA);
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    if (i > 80) i = 0;
    drawProgress(i, "Conectando a WiFi '" + String(WIFI_SSID.c_str()) + "'");
    i += 10;
    Serial.print(".");
  }
  drawProgress(100, "Connectado a WiFi '" + String(WIFI_SSID.c_str()) + "'");
...
void initTime() {
  ...
  while ((now = time(nullptr)) < NTP_MIN_VALID_EPOCH) {
    drawProgress(i * 10, "Actualizando Hora...");
    Serial.print(".");
    delay(300);
    yield();
    i++;
...
/*
  ...
  if (!isFSMounted) {
    ...
    drawProgress(50, "Formateo del sistema de archivos");
    SPIFFS.format();
  }
  drawProgress(100, "Formateo completado");
*/
...
// Creamos una instancia de la clase WiFiManager
  ...
  wifiManager.autoConnect("Clima_Mza");

  bool res;
    // res = wm.autoConnect(); // auto generated AP name from chipid
    // res = wm.autoConnect("AutoConnectAP"); // anonymous ap
    res = wifiManager.autoConnect("Clima_Mza"); // password protected ap
    if(!res)
    {
    ...
    {
        Serial.println("Falla de conexión a la red WIFI");
        gfx.fillBuffer(MINI_BLACK);
        gfx.setFont(ArialRoundedMTBold_14);
        gfx.setTextAlignment(TEXT_ALIGN_CENTER);
        gfx.setColor(MINI_WHITE);
        gfx.drawString(120, 90, "Se Reiniciará por falla de conexión");
        gfx.commit();
        delay(3000);
        //ESP.restart();
    } 
        ...
        Serial.println(String(ESP.getVcc() / 1024.0) + "V"); 
        Serial.println("conectado a la red WIFI");
        WiFi.setAutoReconnect(true);
        WiFi.persistent(true);
        server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) 
        {
          request->send(200, "text/plain", "Hola!!. Para actualizarme debes escribir: IP/update");
        });

        AsyncElegantOTA.begin(&server,"user","12345");    // Start ElegantOTA
        server.begin();
        ...
        gfx.setTextAlignment(TEXT_ALIGN_CENTER);
        gfx.setColor(MINI_WHITE);
        gfx.drawString(120, 90, "Se ha conectado a la red WIFI"  );
        gfx.commit();
        gfx.setFont(ArialRoundedMTBold_14);
        gfx.setTextAlignment(TEXT_ALIGN_CENTER);
        gfx.setColor(MINI_YELLOW);
        NombreRed = WiFi.SSID() ;
        gfx.drawString(120, 115, "Red: "+ NombreRed);
        gfx.commit();
        ...
        gfx.setTextAlignment(TEXT_ALIGN_CENTER);
        gfx.setColor(MINI_YELLOW);
        gfx.drawString(120, 145, "IP Asignada: "+ WiFi.localIP().toString());
        gfx.commit();
        ...
        carousel.disableAllIndicators();//floar PresionAtm = 0;
        initTime();
        drawAbout();
        delay(8000);
...
void updateData() 
{
  ...
  drawProgress(50, "Actualizando condiciones...");
  ...
  drawProgress(70, "Actualizando Pronostico...");
  ...
  drawProgress(80, "Actualizando astronomía...");
...
void drawProgress(uint8_t percentage, String text) 
{
  ...
  //gfx.drawPalettedBitmapFromPgm(20, 5, epd_bitmap_Villa_Logo);
  gfx.setFont(ArialRoundedMTBold_14);
  gfx.setTextAlignment(TEXT_ALIGN_CENTER);
  gfx.setColor(MINI_WHITE);
  gfx.drawString(120, 90, "Bienvenido a Clima Mendoza");
  gfx.setColor(MINI_YELLOW);
...
void drawCurrentWeather() 
{
  ...
  Serial.println(DISPLAYED_LOCATION_NAME);
  ...
  gfx.setColor(MINI_WHITE);
  gfx.setTextAlignment(TEXT_ALIGN_LEFT);
  PresionAtmoString = currentWeather.pressure;
  gfx.drawString(0, 135, "Pres: " + PresionAtmoString + " hPa");


  gfx.setFont(ArialRoundedMTBold_14);
  gfx.setColor(MINI_WHITE);
  gfx.setTextAlignment(TEXT_ALIGN_RIGHT);
  Visi = currentWeather.visibility/1000;
  gfx.drawString(240, 135, "Vis: " + String(Visi) + " km");
  //gfx.commit();
...
void drawAstronomy() 
{
  ...
  gfx.drawString(55, 276, getTime(&time));
  time = currentWeather.sunset;
  gfx.drawString(5, 291, SUN_MOON_TEXT[2] + ":");
  gfx.drawString(55, 291, getTime(&time));
  ...
  gfx.drawString(190, 276, "Hum:"); //SUN_MOON_TEXT[4] + ":");
  gfx.setColor(MINI_WHITE);
  gfx.drawString(235, 276, String((int) currentWeather.humidity) +" %"); //String(moonData.age, 1) + "d");
  
  gfx.setColor(MINI_WHITE);
  gfx.drawString(180, 291, "Vie:"); //SUN_MOON_TEXT[5] + ":");

  VelocFloat=currentWeather.windSpeed*3.6;
  VelocInt=(int) VelocFloat;
  gfx.setColor(MINI_WHITE);
  gfx.drawString(235, 291, String(VelocInt)+" Km/h" ); //String(moonData.illumination * 100, 0) + "%");
...
void drawCurrentWeatherDetail() 
{
  ...
  //drawLabelValue(6, "Visibility:", String(currentWeather.) + "m");
}
...
void drawAbout() //Pantalla con info del wemos y su estado.
{
  gfx.fillBuffer(MINI_BLACK);
  //gfx.drawPalettedBitmapFromPgm(20, 5, epd_bitmap_Villa_Logo);//ThingPulseLogo);

  gfx.setFont(ArialRoundedMTBold_14);
  gfx.setTextAlignment(TEXT_ALIGN_CENTER);
  gfx.setColor(MINI_WHITE);
  gfx.drawString(120, 90, "Reverse eng. by villafapd");
